...
/*
Выявляем 3 сущности: Отдел, Сотрудники и Задание.
*/
...
(
   ...
   FILENAME = 'D:\Company.mdf', -- Указываем Физическое полное имя файла БД.
...
(
  ...
  FILENAME = 'C:\CompanyDB.ldf', -- Указываем Физическое полное имя файла журнала БД.
...
(
 ...
 (
 EmployeesID int IDENTITY NOT NULL PRIMARY KEY,
...
alter authorization on database::[CompanyDB] to [sa]

alter authorization on database::[StokDB] to [sa]
...
( 
	...
	FILENAME = 'C:\StokDB.ldf',      -- Указываем Физическое полное имя файла журнала БД.
	SIZE = 5MB,                        -- Задаем начальный размер файла журнала БД.
	MAXSIZE = 50MB,                    -- Задаем максимальный размер файла журнала БД.
	FILEGROWTH = 5MB                   -- Задаем значение, на которое будет увеличиваться размер файла журнала БД.
)               
